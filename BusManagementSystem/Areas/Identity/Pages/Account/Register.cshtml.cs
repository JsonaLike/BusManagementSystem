...
namespace BusManagementSystem.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public ReservationSystemContext _reservationsystemcontext;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _userManager.AddToRoleAsync(user, Input.Role); // Add "User" role to newly registered users

                    var userId = await _userManager.GetUserIdAsync(user);
                    if(Input.Role == "Passenger") { 
                    Passenger p = new Passenger();
                    p.Name = Input.Name;
                    p.Phone = Convert.ToInt32(Input.PhoneNumber);
                    p.BirthDate = Input.DateOfBirth;
                    p.user_id = userId;
                    p.Gender = Convert.ToInt32(Input.Gender);
                    p.Balance = 500;
                    _reservationsystemcontext.Passengers.Add(p);
